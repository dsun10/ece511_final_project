...
class UnifiedScheduler(Scheduler):
    __intIQs = [
        ...
        IssueQue(name='intIQ1', inports=6, size=48, oports=[
            IssuePort(fu=[IntALU(), IntMult()],
                      rp=[IntRD(6, 0), IntRD(7, 1)]),
            IssuePort(fu=[IntALU(), IntMult()],
                      rp=[IntRD(8, 0), IntRD(9, 1)]),
            IssuePort(fu=[IntDiv(), IntMisc(), IntCvt()],
                      rp=[IntRD(10, 0), IntRD(11, 1)])
        ]),
    ]
    __memIQs = [
    ...
    __fpIQs = [
        IssueQue(name='fpIQ0', inports=8, size=72, oports=[
            IssuePort(fu=[FP_ALU(), FP_MAC()],
                      rp=[FpRD(0,0), FpRD(1, 0), FpRD(2,0)]),
            IssuePort(fu=[FP_MISC(), FP_SLOW()],
                      rp=[FpRD(2,1), FpRD(5,1)]),
...
class TuneableScheduler(Scheduler):

    def __init__(self, num_int_iqs, num_mem_iqs, num_fp_iqs, num_vec_iqs,
                 int_iq_size, mem_iq_size, fp_iq_size, vec_iq_size,
                 *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.__intIQs = []

        for i in range(num_int_iqs):
            iq_name = 'intIQ' + str(i)
            outports = []
            if (i % 3 == 2):
                outports = [
            ...
            self.__intIQs.append(IssueQue(name=iq_name, inports=2, size=int_iq_size, oports=outports))

        self.__memIQs = []

        for i in range(num_mem_iqs):
            iq_name = ''
            outports = []
            if (i % 3) == 0:
                iq_name = 'load' + str(i // 3)
                outports = [
                    IssuePort(fu=[ReadPort()])
                ]
            elif (i % 3) == 1:
                iq_name = 'store' + str(i // 3)
                outports = [
                    IssuePort(fu=[WritePort()])
                ]
            else:
            ...
            self.__memIQs.append(IssueQue(name=iq_name, inports=2,
                                          size=mem_iq_size, oports=outports))

        self.__fpIQs = []
        for i in range(num_fp_iqs):
            iq_name = 'fpIQ' + str(i)
            outports = []
            if (i % 5 == 0):
                outports = [
            ...
            self.__fpIQs.append(IssueQue(name=iq_name, inports=2,
                                         size=fp_iq_size, oports=outports,
                                         scheduleToExecDelay=3))

        for i in range(num_vec_iqs):
            iq_name = 'vecIQ' + str(i)
            self.__fpIQs.append(IssueQue(name=iq_name, inports=5, size=vec_iq_size, oports=[
                IssuePort(fu=[SIMD_Unit()]),
                IssuePort(fu=[SIMD_Unit()]),
                IssuePort(fu=[SIMD_Unit()]),
                IssuePort(fu=[SIMD_Unit()]),
        ...
        self.IQs = self.__intIQs + self.__memIQs + self.__fpIQs
        self.__int_bank = [i.name for i in self.__intIQs]
        self.__mem_bank = [i.name for i in self.__memIQs]
        self.__fp_bank = [i.name for i in self.__fpIQs]
        self.specWakeupNetwork = [
            SpecWakeupChannel(srcs=self.__int_bank + self.__mem_bank, dsts=self.__int_bank + self.__mem_bank),
            SpecWakeupChannel(srcs=self.__fp_bank, dsts=self.__fp_bank)
        ]

        self.useOldDisp = True
